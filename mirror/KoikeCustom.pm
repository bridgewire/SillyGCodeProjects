...
sub new {
    ...
    $self->{DEBUG} = 10;  # 10 means: no debug messages.
    $self->{p} = defined($args{protocol}) ? $args{protocol} : 'gcode'; # protocol: svg or gcode
    $self->{x} = undef;
    $self->{y} = undef;

    # bounding box.  XXX this doesn't correctly handle args,
    ...
    $self->{moveto_color} = defined($args{moveto_color}) ? $args{moveto_color}  : 'none';
    $self->{cut_color}    = defined($args{cut_color})    ? $args{cut_color}     : $dfltclr;
    $self->{line_color}   = defined($args{line_color})   ? $args{line_color}    : $dfltclr;
    $self->{curve_color}  = defined($args{curve_color})  ? $args{curve_color}   : $dfltclr;
    $self->{matrl_color}  = defined($args{material_color})  ? $args{material_color}   : 'rgb(200,255,255)';
    $self->{clist} = [];

    # offsets and scaling
    $self->{mult}         = defined($args{multsclr})     ? $args{multsclr}      : 1;
    $self->{xoffset}      = defined($args{xoffset})      ? $args{xoffset}       : 0;
    $self->{yoffset}      = defined($args{yoffset})      ? $args{yoffset}       : 0;

    # g-code is a stateful language. retain some state information.
    $self->{LASTGCMD} = '';
...
sub set_colors()
{
    ...
    $self->{moveto_color} = defined($args{moveto_color}) ? $args{moveto_color}  : $self->{moveto_color};
    $self->{cut_color}    = defined($args{cut_color})    ? $args{cut_color}     : $self->{cut_color};
    $self->{line_color}   = defined($args{line_color})   ? $args{line_color}    : $self->{line_color};
    $self->{curve_color}  = defined($args{curve_color})  ? $args{curve_color}   : $self->{curve_color};
    $self->{matrl_color}  = defined($args{material_color})  ? $args{material_color}   : $self->{matrl_color};
}
...
sub arcto()
{
    ...
    my $rdus = undef;
    if( defined($args{radius}) ) { $rdus = $args{radius}; }

    my $sweep = undef;
    #if( defined($args{clockwise}) ) { $sweep = ($args{clockwise} ? 0 : 1); }
    if( defined($args{sweep})     ) { $sweep = $args{sweep}; } # ifdef sweep, ignore clockwise
    #if( ! defined($sweep) || ($sweep != 0 && $sweep != 1) ) { die "sweep or clockwise must be defined\n"; }

    my $largearc = undef;
    if( defined($args{largearc}) ) { $largearc = $args{largearc}; }
    ...
    if( defined($args{cx}) && defined($args{cy}) && ( defined($largearc) || defined($sweep) ) )
    {
    ...
    push($self->{clist},
        {
            cmd=>'a',        clr=>(defined($args{clr}) ? $args{clr} : $self->{line_color}), 
            sox=>$sox,       soy=>$soy,
            tox=>$tox,       toy=>$toy,
...
sub svg_lineto()
{
    my $self = shift;
    my $h = shift;
    printf( '<line x1="%.03f" y1="%.03f" x2="%.03f" y2="%.03f" style="stroke:%s;stroke-width:1" />%s',
...
sub svg_arcto()
{
    ...
    printf( '<path d="M%.03f %.03f A%.03f,%.03f 0 %d,%d %.03f,%.03f" style="stroke:%s;stroke-width:1;fill:none" />%s',
...
sub svg_endpoint()
{
    my $self = shift;
    my $h = shift;
    printf( '<circle cx="%.03f", cy="%.03f", r="3", fill="%s" stroke="none" />%s', 
        (($h->{sox} + $self->{xoffset}) * $self->{mult}),
        (($h->{soy} + $self->{yoffset}) * $self->{mult}),
        $h->{clr}, "\n" );
}
...
sub svg_rectangular_background()
{
    ...
    printf( '<rect x="%.03f", y="%.03f", width="%.03f", height="%.03f", fill="%s" stroke="none" />%s', 
        (($h->{sox} + $self->{xoffset}) * $self->{mult}),
        (($h->{soy} + $self->{yoffset}) * $self->{mult}),
        $wd * $self->{mult},
        $ht * $self->{mult},
...
sub gcode_linear_motion()
{
    ...
    my $xstr = sprintf( '%.03f', ($h->{tox} + $self->{xoffset}) * $self->{mult} );
    my $ystr = sprintf( '%.03f', ($h->{toy} + $self->{yoffset}) * $self->{mult} );

    if( $xstr =~ s/0+$// ){ $xstr =~ s/\.$//; }
    if( $ystr =~ s/0+$// ){ $ystr =~ s/\.$//; }

    printf( '%sX%sY%s%s', $s, $xstr, $ystr, "\n" );
}
...
sub gcode_arcto()
{
    ...
    printf( '%s X%.03f Y%.03f I%.03f J%.03f%s', $g, #  $h->{tox}, $h->{toy}, ($h->{tox} - $h->{sox}), ($h->{toy} - $h->{soy}), "\n" );
...
#    printf( '%s X%.03f Y%.03f R%.03f%s', $g, 
#
#        ($h->{tox} + $self->{xoffset}) * $self->{mult}, 
#        ($h->{toy} + $self->{yoffset}) * $self->{mult}, 
#
#        ($h->{tox} - $h->{sox}) * $self->{mult}, 
#        ($h->{toy} - $h->{soy}) * $self->{mult}
#
#        ($h->{tox} + $self->{xoffset}) * $self->{mult}, 
#        ($h->{toy} + $self->{yoffset}) * $self->{mult}, 
#
...
sub printall()
{
    ...
    {
        if( defined($cmds{$i->{cmd}}) ) { &{ $cmds{$i->{cmd}} }( $self, $i ); }
    }
...
sub print_koike_gcode_start()
{
    ...
    # the asterisk is part of the communications protocol, and the remaining
